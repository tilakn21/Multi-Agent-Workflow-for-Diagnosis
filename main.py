...
app = FastAPI(title="Clinical Case Composer API", version="0.1.0")
...
def route_case_to_apis(case: dict) -> dict:
    summary = {"memory_retrieve": None, "literature_search": None, "scrapper": None}
    ...
    print("[DEBUG] Scrapper: initializing vector DB at", vector_db_path)
    vdb = SimpleVectorDB(storage_path=vector_db_path)
    filtering_prompt = case.get("text", "") or ""
    print(f"[DEBUG] Scrapper: filtering prompt: {filtering_prompt}")
    results = vdb.search(filtering_prompt, top_k=10, expand_context=True)
    print(f"[DEBUG] Scrapper: search results: {results}")
    summary['scrapper'] = results
